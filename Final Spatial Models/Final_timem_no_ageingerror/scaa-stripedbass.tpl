...
//Time varying M, aging erorr , double logisitc in cb fishery
...
DATA_SECTION
  ...
  init_int mblock //number of time blocks fornatural mortality
  ...
  init_vector M(fage,lage) //assumed natural mortality for all ages, use this for age 1
  init_matrix M_bay(1,mblock,fage,lage) //M estimates from Adenas model
  init_matrix M_ocean(1,mblock,fage,lage) //M estimates from Adenas model
  ...
  //aging error matrix
  init_int use_age_err //1, use aging err matrix,2, use identity matrix
 ...
 LOCAL_CALCS
  if(test!=12345)
  {
    ...
    cout << "M bay " << endl << M_bay << endl;
    cout << "M ocean " << endl << M_ocean << endl;
...
PARAMETER_SECTION
  ...
  number pen_ac_sel //ac penalty for fsel
...
FUNCTION calculate_mortality
  ...
  */
  ...
  {
    r=2;
    for(y=fmyear;y<=1999;y++)
    {
      Z(r,t,y)=M_ocean(1); //M divided by 2 in dat file to account for time step
    }
    for(y=2000;y<=2009;y++)
    {
      Z(r,t,y)=M_ocean(2); //M divided by 2 in dat file to account for time step
    }
    for(y=2010;y<=lmyear;y++)
    {
      Z(r,t,y)=M_ocean(3); //M divided by 2 in dat file to account for time step
      //r=2;
      //Z(r,t,y)=M_ocean; //M divided by 2 in dat file to account for time step
      //Z(r,t,y)=M; //M divided by 2 in dat file to account for time step
    }
  }
  //set up M for each year block in the CB
  for(t=1;t<=tstep;t++)
  {
    r=1;
    for(y=fmyear;y<=1999;y++)
    {
      Z(r,t,y)=M_bay(1); //M divided by 2 in dat file to account for time step
    }
    for(y=2000;y<=2009;y++)
    {
      Z(r,t,y)=M_bay(2); //M divided by 2 in dat file to account for time step
    }
    for(y=2010;y<=lmyear;y++)
    {
      Z(r,t,y)=M_bay(3); //M divided by 2 in dat file to account for time step
    }
  }//close y loop
  //cout << Z << endl;
  //now add F to M to get total Z
  for(y=fmyear;y<=lmyear;y++)
  {
    for(t=1;t<=tstep;t++)
    {
...
FUNCTION evaluate_likelihood
  ...
  */
  ...
  //pen_ac_sel=0.;
  pen_ac_sel=0.5*square((log_sf1_ac(1,2)-0.)/0.5)+ //ascending param slope for the first timeblock
             0.5*square((log_sf2_ac(1,2)-4.)/0.5); //ascending param slope
